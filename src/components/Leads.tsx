...
type Source = "Website" | "Phone" | "WhatsApp" | "Email" | "Referral";
type Activity = { title: string; meta: string; tone?: "call" | "deal" | "lost" };
type Quotation = { number: string; value: string; status: "Draft" | "Sent" | "Accepted" };
...
export default function Leads() {
  ...
  return <section className="leads-page">
    ...
    <div className="leads-table-wrap"><table className="leads-table"><thead><tr><th>Name & company</th><th>Source</th><th>Enquiry</th><th>Stage</th><th>Next follow-up</th><th>Assigned to</th></tr></thead><tbody>{filtered.map((lead) => { const label = followUpLabel(lead.followUpAt); return <tr key={lead.id} onClick={() => setDrawer(lead)}><td><b>{lead.name}</b><small>{lead.company} {lead.auto && <em className="lead-auto">Auto-captured</em>}</small></td><td><span className="lead-source">{lead.source}</span></td><td className="lead-enquiry">{lead.enquiry}</td><td><span className={`lead-stage lead-stage--${stageClass(lead.stage)}`}>{lead.stage}</span></td><td className={label.startsWith("Overdue") ? "lead-overdue" : ""}>{label}</td><td><span className="lead-assignee"><i>{initials(lead.assigned)}</i>{lead.assigned}</span></td></tr>; })}</tbody></table>{!filtered.length && <div className="settings-empty"><b>No leads match these filters</b><p>Clear a filter or add a new enquiry to get started.</p><button className="erp-action" onClick={() => setCreating(true)}>+ New lead</button></div>}</div>
    {creating && <NewLead leads={leads} close={() => setCreating(false)} create={addLead} />}
    {drawer && <LeadDrawer lead={drawer} close={() => setDrawer(null)} update={updateLead} />}
...
function LeadDrawer({ lead, close, update }: { lead: Lead; close: () => void; update: (id: string, patch: Partial<Lead>, message?: string) => void }) {
  ...
  const quotation = lead.quotation ?? { number: `QT-2026-${lead.id.slice(-3)}`, value: lead.value || "Value pending", status: "Draft" as const };
  const createQuotation = () => addActivity({ title: `Quotation ${quotation.number} created`, meta: timestamp(), tone: "deal" }, { stage: "Quoted", quotation }, "Quotation created");
  const markWon = () => addActivity({ title: "Lead marked won", meta: timestamp(), tone: "deal" }, { stage: "Won", followUpAt: undefined, quotation: { ...quotation, status: "Accepted" }, customer: { name: lead.company || lead.name, code: `CUS-${lead.id.slice(-4)}` } }, "Lead marked won");
  const confirmLost = () => {
  ...
  return <aside className="lead-drawer" aria-label={`${lead.name} lead details`}>
    ...
    {lead.stage === "Quoted" && <QuotationCard quotation={quotation} open={() => update(lead.id, {}, `Opened ${quotation.number}`)} />}
    {lead.stage === "Won" && <section className="lead-section lead-result-section"><h3>Linked records</h3><QuotationCard quotation={quotation} open={() => update(lead.id, {}, `Opened ${quotation.number}`)} /><div className="lead-customer-card"><span>Customer</span><b>{lead.customer?.name || lead.company}</b><a href={`#${lead.customer?.code || "customer"}`} onClick={(event) => { event.preventDefault(); update(lead.id, {}, "Opened customer record"); }}>{lead.customer?.code || "Customer record"} ↗</a></div></section>}
...
function QuotationCard({ quotation, open }: { quotation: Quotation; open: () => void }) {
